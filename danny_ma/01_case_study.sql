...
SELECT DISTINCT customer_id,
    COUNT(*) OVER (PARTITION BY customer_id) AS TotalVisits
FROM sales
...
SELECT sales.customer_id,
    ...
    MIN(menu.product_name) AS 'menu item'
FROM sales
    LEFT JOIN menu ON sales.product_id = menu.product_id
GROUP BY customer_id;
...
SELECT sales.customer_id,
    SUM(
        CASE
            WHEN sales.order_date BETWEEN members.join_date AND DATEADD(DAY, 7, join_date) THEN (menu.price * 20)
            ELSE CASE
                WHEN menu.product_name != 'sushi' THEN 10 * menu.price
                WHEN menu.product_name = 'sushi' THEN 20 * menu.price
            END
